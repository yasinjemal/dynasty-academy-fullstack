  ...
  try {
    ...
    try {
      ...
      if (error.code === 'P2010' || error.meta?.code === '42P07') {
        ...
        console.log('⚠️  Could not create concepts.embedding index:', error.message);
      }
    }
    ...
    try {
      ...
      if (error.code === 'P2010' || error.meta?.code === '42P07') {
        ...
        console.log('⚠️  Could not create content_embeddings.embedding index:', error.message);
      }
    }
    ...
    if (result.length > 0) {
      console.log(`✅ pgvector ${result[0].extversion} is installed and ready!\n`);
    }
    ...
    console.error('3. You may need to enable pgvector manually in Supabase SQL Editor');

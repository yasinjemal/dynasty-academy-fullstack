...
async function main() {
  ...
  courses.slice(0, 5).forEach((course, i) => {
    console.log(
      `  ${i + 1}. ${course.title} (${course._count.lessons} lessons)`
    );
  });
  if (courses.length > 5) {
  ...
  try {
    ...
    console.log("Results:");
    console.log(
      `  - Total courses processed: ${extractionResults.totalCourses}`
    );
    console.log(
      `  - Total concepts extracted: ${extractionResults.totalConcepts}`
    );
    console.log(
      `  - Total relationships mapped: ${extractionResults.totalRelationships}`
    );
    console.log(`  - Total cost: $${extractionResults.totalCost.toFixed(4)}`);
    console.log(
      `  - Duration: ${(extractionResults.duration / 1000).toFixed(1)}s\n`
    );
  } catch (error) {
  ...
  try {
    ...
    testResults.tests.forEach((test, i) => {
      ...
      console.log(
        `   Pass Rate: ${test.passed}/${test.passed + test.failed}\n`
      );
    });
    ...
    if (testResults.passedAllTests) {
      ...
      console.log(
        "   Consider extracting more concepts or adjusting parameters\n"
      );
    }
  } catch (error) {

...
export default function IntelligentBookUploader({ onComplete }: IntelligentBookUploaderProps) {
  const [file, setFile] = useState<File | null>(null)
  const [isProcessing, setIsProcessing] = useState(false)
  const [currentStage, setCurrentStage] = useState(0)
  const [stages, setStages] = useState<ProcessingStage[]>([
    { id: 'upload', label: 'Uploading Book File', status: 'pending', icon: Upload },
    { id: 'extract', label: 'Extracting Book Content', status: 'pending', icon: FileText },
    { id: 'metadata', label: 'Analyzing Metadata (Title, Author)', status: 'pending', icon: BookOpen },
    { id: 'content', label: 'AI Content Analysis (GPT-4)', status: 'pending', icon: Brain },
    { id: 'description', label: 'Generating Description', status: 'pending', icon: Wand2 },
    { id: 'category', label: 'Smart Category Detection', status: 'pending', icon: Tag },
    { id: 'tags', label: 'Extracting Keywords & Tags', status: 'pending', icon: Sparkles },
    { id: 'audience', label: 'Identifying Target Audience', status: 'pending', icon: Users },
    { id: 'pricing', label: 'Calculating Optimal Price', status: 'pending', icon: DollarSign },
    { id: 'cover', label: 'Processing/Generating Cover', status: 'pending', icon: ImageIcon },
    { id: 'seo', label: 'Creating SEO Metadata', status: 'pending', icon: TrendingUp },
    { id: 'finalize', label: 'Finalizing Book Data', status: 'pending', icon: CheckCircle2 }
  ])
  ...
  const updateStage = (index: number, status: ProcessingStage['status'], result?: any) => {
  ...
  const processBook = async () => {
    ...
    try {
      ...
      updateStage(0, 'complete', { fileUrl: uploadData.fileUrl, fileId: uploadData.fileId })
      ...
      updateStage(3, 'complete', { themes: aiData.themes, topics: aiData.topics })
      await new Promise(resolve => setTimeout(resolve, 300))
      ...
      await new Promise(resolve => setTimeout(resolve, 800))
      updateStage(5, 'complete', { category: aiData.category, confidence: aiData.categoryConfidence })
      await new Promise(resolve => setTimeout(resolve, 300))
          ...
          <CardContent className="pt-6">
            ...
            >
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-4">
                ...
                <span className="text-sm">Supports: PDF, DOCX, EPUB, TXT, MD</span>
              </p>
                ...
                return (
                  <div
                    ...
                    className={`
                      ...
                      ${stage.status === 'processing' ? 'bg-purple-100 dark:bg-purple-900/20 border-2 border-purple-500' : ''}
                      ${stage.status === 'complete' ? 'bg-green-50 dark:bg-green-950/20' : ''}
                      ${stage.status === 'error' ? 'bg-red-50 dark:bg-red-950/20' : ''}
                      ${stage.status === 'pending' ? 'opacity-50' : ''}
                    `}
                  >
                    <div className="mt-0.5">
                      {stage.status === 'pending' && <Icon className="h-5 w-5 text-gray-400" />}
                      {stage.status === 'processing' && <Loader2 className="h-5 w-5 text-purple-600 animate-spin" />}
                      {stage.status === 'complete' && <CheckCircle2 className="h-5 w-5 text-green-600" />}
                      {stage.status === 'error' && <AlertCircle className="h-5 w-5 text-red-600" />}
                    </div>
                    <div className="flex-1">
                      <p className={`font-medium ${stage.status === 'processing' ? 'text-purple-900 dark:text-purple-100' : ''}`}>
                        {stage.label}
                      </p>
                      {stage.result && stage.status === 'complete' && (
                        <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                          {stage.id === 'extract' && `${stage.result.totalPages} pages • ${stage.result.wordCount?.toLocaleString()} words`}
                          {stage.id === 'metadata' && `${stage.result.title} by ${stage.result.author}`}
                          {stage.id === 'category' && `${stage.result.category} (${stage.result.confidence}% confidence)`}
                          {stage.id === 'tags' && stage.result.tags?.slice(0, 3).join(', ')}
                          {stage.id === 'pricing' && `$${stage.result.suggestedPrice}`}
                        </p>
                      )}
                    </div>
                  </div>
        ...
        <Card className="border-green-200 dark:border-green-800">
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">${bookData.price}</p>
                  {bookData.priceReasoning && (
                    <p className="text-xs text-gray-500 mt-1">{bookData.priceReasoning}</p>
                  )}
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-600 dark:text-gray-400">Pages / Reading Time</label>
                  <p>{bookData.totalPages} pages • ~{bookData.readingTime} min read</p>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <p className="mt-2 text-gray-700 dark:text-gray-300">{bookData.targetAudience}</p>
            </div>
            ...
            <div className="flex gap-3 pt-4 border-t">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  setBookData(null)
                  setShowPreview(false)
                  setStages(stages.map(s => ({ ...s, status: 'pending', result: undefined })))
                }}
                size="lg"
              >
                Start Over

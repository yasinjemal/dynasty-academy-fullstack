...
import { 
  Zap, Flame, Star, Crown, Sparkles, 
  Clock, Users, TrendingUp, Gift, Lock 
} from 'lucide-react';
...
const rarityConfig = {
  common: { color: 'from-gray-400 to-gray-600', bgColor: 'bg-gray-500/20', borderColor: 'border-gray-500/50', textColor: 'text-gray-400' },
  rare: { color: 'from-blue-400 to-blue-600', bgColor: 'bg-blue-500/20', borderColor: 'border-blue-500/50', textColor: 'text-blue-400' },
  epic: { color: 'from-purple-400 to-purple-600', bgColor: 'bg-purple-500/20', borderColor: 'border-purple-500/50', textColor: 'text-purple-400' },
  legendary: { color: 'from-orange-400 via-yellow-400 to-orange-400', bgColor: 'bg-gradient-to-br from-orange-500/20 to-yellow-500/20', borderColor: 'border-orange-500/50', textColor: 'text-orange-400' }
};

export default function PowerUpsSystem({ powerUps, events, userCoins }: PowerUpsProps) {
  const [activePowerUps, setActivePowerUps] = useState(powerUps.filter(p => p.isActive));
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6 bg-gradient-to-r from-yellow-500/10 via-orange-500/10 to-yellow-500/10 border-yellow-500/20">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Boost your reading progress</p>
          </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {activePowerUps.map((powerUp, index) => (
              <ActivePowerUpCard key={powerUp.id} powerUp={powerUp} index={index} />
            ))}
          </div>
        </div>
      )}
      ...
      {events.filter(e => e.isActive).length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {events.filter(e => e.isActive).map((event, index) => (
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {powerUps.filter(p => !p.isActive).map((powerUp, index) => (
            <PowerUpCard 
              key={powerUp.id} 
              powerUp={powerUp} 
              index={index}
...
function ActivePowerUpCard({ powerUp, index }: { powerUp: PowerUp; index: number }) {
  const Icon = powerUp.icon;
  const config = rarityConfig[powerUp.rarity];
  
  // Calculate time remaining
  const timeRemaining = powerUp.expiresAt ? powerUp.expiresAt.getTime() - Date.now() : 0;
  ...
  return (
    ...
    >
      <Card className={`p-5 border-2 ${config.borderColor} ${config.bgColor} relative overflow-hidden`}>
        {/* Animated shimmer */}
        <motion.div
...
function PowerUpCard({ powerUp, index, userCoins }: { powerUp: PowerUp; index: number; userCoins: number }) {
  const Icon = powerUp.icon;
  const config = rarityConfig[powerUp.rarity];
      ...
      <Card className={`p-5 border-2 ${config.borderColor} ${config.bgColor} relative overflow-hidden`}>
        <div className="relative">
          ...
          <div className="flex items-center gap-3 mb-3">
            <div className={`p-3 rounded-xl bg-gradient-to-br ${config.color} flex items-center justify-center`}>
              <Icon className="w-6 h-6 text-white" />
            </div>

            <div>
              <div className="font-bold">{powerUp.name}</div>
              <Badge variant="outline" className={`${config.textColor} ${config.borderColor} text-xs capitalize`}>
                {powerUp.rarity}
              </Badge>
            </div>
          </div>
      ...
      <Card className="p-5 border-2 border-pink-500/50 bg-gradient-to-br from-pink-500/20 to-purple-500/20 relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <div className="flex items-center gap-2 text-sm text-pink-500 font-medium">
            <Clock className="w-4 h-4" />
            <span>Ends in {daysRemaining}d {hoursRemaining}h</span>
          </div>
        </div>
      </Card>
    </motion.div>

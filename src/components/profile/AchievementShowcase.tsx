...
import { 
  Trophy, Star, Flame, BookOpen, Users, Zap, 
  Crown, Sparkles, Award, Target, Lock 
} from 'lucide-react';
...
export default function AchievementShowcase({ achievements, totalPossible }: AchievementShowcaseProps) {
...
function AchievementCard({ achievement, index, isUnlocked }: { 
  achievement: Achievement; 
  index: number;
  isUnlocked: boolean;
}) {
    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            ...
            `}>
              ...
              <div className="relative">
                <motion.div
                  className={`
                    ...
                    ${isUnlocked ? `bg-gradient-to-br ${config.color}` : 'bg-muted'}
                ...
                >
                  <Icon className={`w-8 h-8 ${isUnlocked ? 'text-white' : 'text-muted-foreground'}`} />
                </motion.div>
              ...
              {/* Name */}
              <div className={`text-center text-sm font-semibold ${isUnlocked ? '' : 'text-muted-foreground'}`}>
        ...
        <TooltipContent side="top" className="max-w-xs">
          <div>
            <div className="font-bold mb-1">{isUnlocked ? achievement.name : 'Locked Achievement'}</div>

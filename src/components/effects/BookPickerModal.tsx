...
import { X, BookOpen, Star, Zap, Crown, TrendingUp, Sparkles } from 'lucide-react'
...
export default function BookPickerModal({ isOpen, onClose }: BookPickerModalProps) {
                    ...
                    <div className="relative backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-6 h-full">
                      ...
                      >
                        <div className="w-full h-full bg-[#0A0E27] rounded-xl flex items-center justify-center">
                          <BookOpen className={`w-8 h-8 bg-gradient-to-br ${book.color} bg-clip-text text-transparent`} />
                        </div>
                      </motion.div>
                      
                      {/* Category Badge */}
                      <div className={`inline-block bg-gradient-to-r ${book.color} px-2 py-1 rounded-md mb-3`}>
                        <span className="text-xs font-semibold text-white">{book.category}</span>
                      </div>
                      ...
                      {/* Floating Particles */}
                      {hoveredBook === book.id && [...Array(3)].map((_, i) => (
                        <motion.div
                          key={i}

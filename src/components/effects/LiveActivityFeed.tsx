...
export default function LiveActivityFeed() {
  ...
  useEffect(() => {
    ...
    const actions = [
      {
        action: "completed a book",
        points: 50,
        icon: "zap" as const,
        color: "from-orange-500 to-yellow-500",
      },
      {
        action: "reached 30-day streak",
        points: 100,
        icon: "flame" as const,
        color: "from-red-500 to-orange-500",
      },
      {
        action: "became Premium",
        points: 200,
        icon: "crown" as const,
      ...
      <div className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-4 shadow-2xl">
            ...
            {events.map((event) => {
              ...
              return (
                ...
                >
                  {/* Gradient Background */}
                  <div
                    className={`absolute inset-0 bg-gradient-to-r ${event.color} opacity-10`}
                  />
        ...
        >
          <p className="text-xs text-white/40">
            <span className="font-semibold text-white/60">12.5K+</span> Dynasty
            members active now
          </p>
        </motion.div>
      </div>

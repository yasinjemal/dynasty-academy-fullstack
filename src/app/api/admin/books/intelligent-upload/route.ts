  ...
  try {
    ...
    try {
      if (file.type === 'application/pdf') {
        // Extract PDF content
        extractedData = { ...extractedData, ...(await extractPDFContent(buffer, file.name)) }
      } else if (file.type === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
        // Extract DOCX content
        extractedData = { ...extractedData, ...(await extractDOCXContent(buffer, file.name)) }
      } else if (file.type === 'text/plain' || file.type === 'text/markdown') {
        // Extract text content
        extractedData = { ...extractedData, ...(await extractTextContent(buffer.toString(), file.name)) }
      }
    } catch (error) {
...
// Extract PDF content
async function extractPDFContent(buffer: Buffer, fileName: string): Promise<any> {
...
// Extract DOCX content
async function extractDOCXContent(buffer: Buffer, fileName: string): Promise<any> {
  try {
    ...
    const title = lines[0]?.trim() || fileName.replace(/\.docx$/i, '').replace(/_/g, ' ')
    
    // Try to extract author (look for common patterns)
    const authorMatch = text.match(/(?:by|author|written by)[\s:]+([A-Z][a-z]+\s+[A-Z][a-z]+)/i)
...
async function extractTextContent(text: string, fileName: string): Promise<any> {
  const words = text.split(/\s+/).filter(w => w.length > 0)
  const lines = text.split('\n').filter(l => l.trim().length > 0)
  
  // Extract title (first line or first heading)
  let title = lines[0]?.replace(/^#+\s*/, '').trim() || fileName.replace(/\.(txt|md)$/i, '')

...
export default function AdminEmbeddingControlPanel() {
  ...
  const [selectedTypes, setSelectedTypes] = useState<string[]>(['courses', 'lessons', 'questions', 'books']);
  const [mode, setMode] = useState<'all' | 'unembedded' | 'retry'>('unembedded');
  ...
  const contentTypes = [
    ...
    { id: 'questions', name: 'Quiz Questions', icon: HelpCircle, color: 'bg-purple-500' },
    ...
    <div className="space-y-6 p-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Generate and manage vector embeddings for AI-powered search and recommendations
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {stats ? `${stats.embedded.toLocaleString()} of ${stats.total.toLocaleString()} items embedded` : 'Loading...'}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center">
                <div className="text-3xl font-bold text-green-600">{stats?.embedded || 0}</div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-muted-foreground">
                          {stats ? stats[type.id as keyof EmbeddingStats].toLocaleString() : '...'} items
                        </div>
                      </div>
                    </div>
                    <Checkbox
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card
              className={`cursor-pointer transition-all ${
                mode === 'unembedded' ? 'ring-2 ring-green-500' : 'hover:shadow-md'
              }`}
            ...
            >
              <CardContent className="p-6 text-center">
                ...
                <Badge variant="outline" className="mt-3">Full Refresh</Badge>
              </CardContent>
            </Card>
            ...
            >
              <CardContent className="p-6 text-center">
                ...
                <Badge variant="outline" className="mt-3">Error Recovery</Badge>
              </CardContent>
            </Card>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Embed specific content types individually</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {contentTypes.map((type) => (
            ...
            <div className="flex-1">
              ...
              <div className="text-sm text-muted-foreground">
                {selectedTypes.length === 0 ? (
                  ...
                  `Will embed ${selectedTypes.length} content type${selectedTypes.length > 1 ? 's' : ''} in "${mode}" mode`
                )}
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              {progress.errors.length > 0 && (
                <Alert variant="destructive">
                  ...
                  <AlertDescription>
                    {progress.errors.length} error(s) occurred. Check logs for details.
                  </AlertDescription>
                </Alert>
              )}
            </div>
      ...
      <Alert>
        ...
        <AlertDescription>
          <strong>What are embeddings?</strong> Vector embeddings convert your content into numerical representations 
          that enable AI-powered semantic search, intelligent recommendations, and adaptive learning paths. 
          This powers the Dynasty Nexus 2.0 Self-Healing Knowledge Graph.
        </AlertDescription>
      </Alert>
    </div>
  );

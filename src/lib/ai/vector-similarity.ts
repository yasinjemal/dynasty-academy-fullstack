  ...
  try {
    ...
    const results = await prisma.$queryRaw<Array<{
      ...
      SELECT 
        ...
        difficulty_score,
        category,
        embedding ${Prisma.raw(operator)} ${Prisma.raw(`'${embeddingStr}'::vector`)} AS distance,
        1 - (embedding <=> ${Prisma.raw(`'${embeddingStr}'::vector`)}) AS similarity,
        metadata
      FROM concepts
      WHERE embedding IS NOT NULL
      ORDER BY embedding ${Prisma.raw(operator)} ${Prisma.raw(`'${embeddingStr}'::vector`)} ${Prisma.raw(orderDirection)}
      LIMIT ${limit};
    `;
...
export async function findSimilarContent(
  ...
  // Check cache first
  const cacheKey = `similar_content:${contentType}:${hashEmbedding(embedding)}:${limit}`;
...
export async function semanticConceptSearch(
  ...
  try {
    ...
    const { embedding } = await generateEmbedding(queryText, 'query', 'concept_search');
...
export async function getConceptRecommendations(
  ...
  try {
    ...
    const recommendations: Array<{ conceptId: string; score: number; reason: string }> = [];
    ...
    if (concept.embedding) {
      ...
      if (embeddingArray) {
        const similar = await findSimilarConcepts(embeddingArray, limit * 2, 'cosine', 0.7);
...
export async function getVectorSearchStats(): Promise<{
  ...
  try {
    const [totalConcepts, conceptsWithEmbeddings, totalContentEmbeddings] = await Promise.all([
